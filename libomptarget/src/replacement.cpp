...
void dumpTargetData(HostDataToTargetListTy *DataList) {
  ...
  for (auto &HT : *DataList) {
    LLD_DP("Entry %2d: Base=" DPxMOD
           ", Valid=%d, Deleted=%d, Reuse=%ld, Time=%lu, ReuseDist=%lu, "
           "Locality=%.1f, Size=%" PRId64 ", DevSize=%" PRId64
           " Type=0x%" PRIx64 "\n",
           i, DPxPTR(HT.HstPtrBegin), HT.IsValid, HT.IsDeleted, HT.Reuse,
           HT.TimeStamp, HT.ReuseDist, HT.Locality,
...
int64_t placeDataObj(DeviceTy &Device, HostDataToTargetTy *Entry, int32_t idx,
                     int64_t &MapType, int64_t Size, void *Base, uint64_t LTC,
                     bool data_region) {
  if (GMode == -4) {
    MapType |= OMP_TGT_MAPTYPE_SDEV;
    return Size;
...
int64_t releaseDataObj(DeviceTy &Device, HostDataToTargetTy *E) {
  ...
  if (PreMap == MEM_MAPTYPE_DEV) {
    ...
    if (E->IsValid) {
      int rt = Device.data_retrieve((void *)E->HstPtrBegin,
                                    (void *)E->TgtPtrBegin, Size);
    ...
    LLD_DP("  Replace " DPxMOD " from UM, size=%ld\n", DPxPTR(E->HstPtrBegin),
           Size);
    Device.RTL->data_opt(Device.RTLDeviceID, Size, (void *)E->HstPtrBegin,
                         0); // pin to host
    Device.RTL->data_opt(Device.RTLDeviceID, Size, (void *)E->HstPtrBegin,
                         5); // prefetch to host
    Device.umSize -= Size;
...
inline bool comparePriority(int64_t MapType, int64_t Size, uint64_t LTC,
                            HostDataToTargetTy *R) {
  if (GMode >= -1) {
...
bool rankArgs(std::pair<int32_t, int64_t> A, std::pair<int32_t, int64_t> B,
              DeviceTy &Device, int64_t *arg_sizes) {
  if (GMode == -2) {
    double AL = (double)getLocalReuse(A.second) * Device.loopTripCnt /
                arg_sizes[A.first];
    double BL = (double)getLocalReuse(B.second) * Device.loopTripCnt /
                arg_sizes[B.first];
    return (AL > BL);
  } else if (GMode == -1) {
...
// replace a data object
int64_t replaceDataObj(DeviceTy &Device, HostDataToTargetTy *Entry, int32_t idx,
                       int64_t &MapType, int64_t Size, int64_t AvailSize,
                       void *Base, uint64_t LTC, bool data_region) {
  if (GMode == -4) { // in no rank mode, do not replace
    MapType |= OMP_TGT_MAPTYPE_UVM;
    return 0;
  }
  ...
  for (auto &HT : Device.HostDataToTargetMap) {
    ...
    mem_map_type PreMap = getMemMapType(HT.MapType);
    if ((GMode == -2 && PreMap < MEM_MAPTYPE_PART) ||
        (GMode == -3 && (PreMap < MEM_MAPTYPE_PART &&
                         comparePriority(MapType, Size, LTC, &HT))) ||
        (GMode >= -1 && (PreMap < MEM_MAPTYPE_PART &&
                         comparePriority(MapType, Size, LTC, &HT))) ||
  ...
  // sort replace list based on the selected criterion.
  ...
  // place data
  return placeDataObj(Device, Entry, idx, MapType, Size, Base, LTC,
                      data_region);
}
...
target_uvm_data_mapping_opt(DeviceTy &Device, void **args_base, void **args,
  ...
  if (data_region)
    LLD_DP("DATA\t\t\t\t(#iter: %lu    device: %ld    UM: %ld) at %lu\n", ltc,
           Device.deviceSize, Device.umSize, GlobalTimeStamp)
  else
    LLD_DP("COMPUTE (" DPxMOD
           ")\t(#iter: %lu    device: %ld    UM: %ld) at %lu\n",
  ...
  if (GMode == 0) { // cluster
    ...
    if (RSize > 0 && Device.CurrentCluster) {
      if (Device.IsNewCluster)
        ...
        assert(Device.CurrentCluster->Size == CSize &&
               "The size of cluster should be consistent.");
      replaceDataCluster(
          Device, RSize, total_dev_size - Device.deviceSize - Device.umSize,
          argList, LRs, new_arg_types, new_arg_sizes, args_base, ltc);
    } else if (RSize > 0) {
      for (auto I : argList) {
        int32_t idx = I.first;
        LookupResult lr = LRs[idx];
        if (lr.Entry != Device.HostDataToTargetMap.end() &&
            lr.Entry->Irreplaceable) {
          lr.Entry->Irreplaceable = false;
          continue;
        }
        LLD_DP("  Arg %d (" DPxMOD ") mapping is not decided\n", idx,
               DPxPTR(args_base[idx]));
    ...
    for (auto I : argList) {
      ...
      if (HT && HT->IsValid && getMemMapType(HT->MapType) != MEM_MAPTYPE_PART)
        HT->Irreplaceable = true;
      int64_t AvailSize =
          total_dev_size - used_dev_size - Device.deviceSize - Device.umSize;
      assert(AvailSize > -1024); // reserve space for non UM variables
      if (HT && getMemMapType(HT->MapType) == MEM_MAPTYPE_PART)
        AvailSize += HT->DevSize;
      //if (GMode <= -2 && data_region) // in reuse distance and local management, do not map in target data region
      if (data_region)
        placeDataObj(Device, HT, idx, new_arg_types[idx], DataSize,
                     args_base[idx], ltc, data_region);
      else if (DataSize <= AvailSize)
        used_dev_size +=
            placeDataObj(Device, HT, idx, new_arg_types[idx], DataSize,
                         args_base[idx], ltc, data_region);
      else {
        int64_t allocateSize =
            replaceDataObj(Device, HT, idx, new_arg_types[idx], DataSize,
                           AvailSize, args_base[idx], ltc, data_region);
        used_dev_size += allocateSize;
        if (PartialMap && allocateSize == 0 && partial_idx == -1) {
          partial_idx = idx;
          partial_HT = HT;
    ...
    if (PartialMap && partial_idx >= 0) {
      ...
      if (partial_HT && getMemMapType(partial_HT->MapType) == MEM_MAPTYPE_PART)
        AvailSize += partial_HT->DevSize;
      replaceDataObjPart(Device, partial_HT, partial_idx,
                         new_arg_types[partial_idx], new_arg_sizes[partial_idx],
                         AvailSize, args_base[partial_idx], ltc, data_region);
    }
