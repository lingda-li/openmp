...
EXTERN void __tgt_target_data_mapper(int64_t device_id, int32_t arg_num,
                                     void **args_base, void **args,
                                     int64_t *arg_sizes, int64_t *arg_types,
                                     void **arg_mapper_ptrs) {
  if (IsOffloadDisabled()) return;

  DP("Entering data region for device %" PRId64 " with %d mappings\n",
     device_id, arg_num);
  ...
  int rc = target_data(Device, arg_num, args_base, args, arg_sizes, arg_types,
                       arg_mapper_ptrs);
  HandleTargetOutcome(rc == OFFLOAD_SUCCESS);
}
